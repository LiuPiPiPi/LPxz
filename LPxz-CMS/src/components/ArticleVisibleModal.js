...
const ArticleVisibleModal = memo(forwardRef((props, ref) => {
    ...
    useEffect(() => {
        if (article !== null) {
            let published = article.published ? (article.password !== '' ? 'c' : 'a') : 'b'
            form.setFieldsValue({ ...article, published })
        } else {
            form.setFieldsValue({ published: 'a' })
        }
    }, [article, form])

    const handleSubmit = () => {
        if (form.getFieldValue('published') === 'c' && (form.getFieldValue('password') === '')) {
            message.error("密码保护模式必须填写密码！")
        } else {
            let values = {
                ...form.getFieldsValue(),
                ...{ published: form.getFieldValue('published') === 'b' ? false : true }
            }
